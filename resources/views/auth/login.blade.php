...
<div class="container">
    <div class="screen">
        <div class="screen__content">
            <h2 class="login__field" style="text-align: left; padding: 20px 10px" >{{ __('LOG IN') }}</h2>
            <form method="POST" action="{{ route('login') }}">
                @csrf

                <div class="login__field">
                    <i class="login__icon fa fa-user"></i>
                    <input id="email" class="login__input" type="email" name="email" :value="old('email')" placeholder="{{ __('Email') }}" required autofocus autocomplete="username" />
                    <x-input-error :messages="$errors->get('email')" class="custom-error" />
                </div>

                <div class="login__field">
                    <i class="login__icon fa fa-lock"></i>
                    <input id="password" class="login__input" type="password" name="password" placeholder="{{ __('Password') }}" required autocomplete="current-password" />
                    <x-input-error :messages="$errors->get('password')" class="custom-error" />
                </div>

                <div class="login__field" style="text-align: left; padding: 20px 10px" >
